...
各出品者は、リアルタイム KPI（売上、注文、商品、顧客）、収益内訳（売上総額、配送コスト、純利益）、および注文・商品・承認管理が一体となったセルフサービス管理パネルを利用できます。

![出品者ダッシュボード](./docs/screenshots/VastyDash.png)
...
プラットフォーム全体の配送業務を一元管理：配送方法とゾーンの設定、配送パートナーの登録と管理、稼働中の発送のトラッキング、平均配送時間や配送中の件数などの KPI を一目で確認できます。

![配送管理](./docs/screenshots/VastyDelivery.png)

## ストアフロントビルダー

出品者はドラッグ＆ドロップのページビルダーでストアフロントをデザインできます。ヒーローバナー、注目商品、カテゴリ、お客様の声、カスタムページ — すべての変更を即座にサイドバイサイドで反映するライブプレビュー付き。

![ストアフロントビルダー](./docs/screenshots/VastyAppPreview.png)
