<p align="center">
  ...
  <p align="center">
    <a href="https://vasty.shop"><strong>🌐 جرّب العرض التجريبي المباشر →</strong></a>
  </p>
</p>
...
<p align="center">
  <a href="https://vasty.shop">العرض التجريبي المباشر</a> |
...
- [ما هو Vasty Shop؟](#ما-هو-vasty-shop)
- [لماذا Vasty Shop؟](#لماذا-vasty-shop-مقارنة)
- [لوحة البائع](#لوحة-البائع)
- [البدء السريع](#البدء-السريع)
- [الميزات](#الميزات)
- [إدارة التوصيل](#إدارة-التوصيل)
- [منشئ واجهة المتجر](#منشئ-واجهة-المتجر)
...
يحصل كل بائع على لوحة تحكم ذاتية بمؤشرات أداء فورية (الإيرادات، الطلبات، المنتجات، العملاء)، تفاصيل الأرباح — إجمالي المبيعات، تكاليف التوصيل، صافي الربح — وإدارة كاملة للطلبات والمنتجات والموافقات.

![لوحة البائع](./docs/screenshots/VastyDash.png)
...
كل ما يلزم لتشغيل متجر إلكتروني — كتالوج، سلة، إتمام الشراء، وما بعد الشراء.
...
### المدفوعات والتمويل
مدفوعات متعددة المزودين مع عمولات منصة تلقائية، دفعات للبائعين، وتتبع مالي.
...
### التسويق والنمو
أدوات توليد الطلب مدمجة — لا حاجة إلى إضافات طرف ثالث.
...
### العمليات
سير عمل في المتجر وإدارة الطلبات للبائعين الذين يبيعون خارج الويب.
...
### المنصة
قدرات تأسيسية تدعم كل الوحدات الأخرى.
...
عمليات التوصيل والشحن على مستوى المنصة في مكان واحد: قم بإعداد طرق ومناطق الشحن، وسجّل شركاء التوصيل وأدرهم، وتتبّع الشحنات النشطة، وراقب مؤشرات الأداء مثل متوسط وقت التوصيل وعدد الشحنات أثناء النقل بنظرة واحدة.

![إدارة التوصيل](./docs/screenshots/VastyDelivery.png)

## منشئ واجهة المتجر

يصمّم البائعون واجهة متجرهم باستخدام منشئ صفحات بالسحب والإفلات — شرائط البطل، المنتجات المميزة، الفئات، الشهادات، والصفحات المخصصة — مع معاينة مباشرة تُظهر كل تغيير جنباً إلى جنب فور حدوثه.

![منشئ واجهة المتجر](./docs/screenshots/VastyAppPreview.png)
...
تم بناء Vasty Shop للاستضافة الذاتية. يمكن تشغيله على أي مضيف يدعم Docker — خادم VPS، أو Render/Railway/Fly، أو مجموعة Kubernetes خاصة بك.

**مسارات النشر الشائعة:**

- **Docker على مضيف واحد** — ملف `docker-compose.yml` المُضمَّن صالح للإنتاج مع بعض التعديلات على متغيرات البيئة (تغيير أهداف البناء، استخدام وحدات تخزين مُدارة، وضع reverse proxy أمامه).
- **Postgres + Redis مُدارة** — وجّه `DATABASE_URL` / `REDIS_HOST` إلى نسخة مُدارة وشغّل فقط حاويات الـ backend/frontend.
- **إعداد المزودين** — التخزين، البريد، المدفوعات، البحث، الرسائل النصية، والذكاء الاصطناعي كلها قابلة للتوصيل عبر متغيرات البيئة. الفهرس الكامل للمزودين وأدلة الإعداد: [`backend/docs/providers/`](backend/docs/providers).

**قائمة التحقق قبل الإطلاق:**

- [ ] غيّر كلمة مرور المسؤول الافتراضية (`admin@vasty.shop` / `admin123`)
- [ ] عيّن قيمة عشوائية قوية لـ `JWT_SECRET`
- [ ] عيّن `NODE_ENV=production` في الـ backend
- [ ] اضبط مزوّد بريد إلكتروني واحد على الأقل للرسائل المعاملاتية
- [ ] اضبط مزوّد تخزين (S3/R2/MinIO) للوسائط المرفوعة
- [ ] استخرج أسرار `backend/.env` من المستودع ومن التحكم بالإصدار

## سجل التغييرات

راجع [CHANGELOG.md](CHANGELOG.md) لسجل الإصدارات والتغييرات البارزة.
