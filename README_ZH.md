...
每位商户都可获得自助式控制面板，内置实时 KPI（营收、订单、商品、客户）、收益明细（总销售额、配送成本、净利润），以及完整的订单、商品与审批管理。

![商户仪表盘](./docs/screenshots/VastyDash.png)
...
全平台配送运营集中管理：配置配送方式与配送区域、注册和管理配送员、跟踪进行中的发货，以及一眼掌握平均配送时长、在途件数等关键指标。

![配送管理](./docs/screenshots/VastyDelivery.png)

## 店铺编辑器

商户通过拖放式页面构建器设计自己的店铺 — 英雄横幅、精选商品、分类、用户评价与自定义页面 — 侧边实时预览面板即时反映每一次改动。

![店铺编辑器](./docs/screenshots/VastyAppPreview.png)
