/**
 * StorefrontLoginPage - Store-specific customer login
 * Provides a branded login experience for each store
 */

import React, { useState, useEffect } from 'react';
import { useParams, useNavigate, useLocation, Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import { Mail, Lock, Eye, EyeOff, Store, Loader2, ArrowLeft } from 'lucide-react';
...
export function StorefrontLoginPage() {
  ...
  const { storeLogin, isStoreAuthenticated } = useStoreAuth();
  ...
  // Get the return path from location state (handles both string and object with pathname)
  const fromState = (location.state as any)?.from;
  const from = typeof fromState === 'string' ? fromState : (fromState?.pathname || `/store/${shopId}`);
