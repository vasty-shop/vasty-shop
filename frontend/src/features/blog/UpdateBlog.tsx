...
import { useBlogPostById, useBlogCategories, useUpdateBlogPost, useUploadBlogImages, useCreateCategory } from './hooks/useBlog';
import { BlogSidebar } from './components';
import { UpdateBlogPostDto } from './types';
...
const UpdateBlog: React.FC = () => {
  ...
  const { data: post, isLoading: postLoading, error: postError } = useBlogPostById(id || null);
  ...
  const handleSubmit = async (newStatus: 'draft' | 'published') => {
    ...
    const postData: UpdateBlogPostDto = {
      title: title.trim(),
    ...
    try {
      const updatedPost = await updatePostMutation.mutateAsync({ id, data: postData });
      toast.success(newStatus === 'published' ? 'Post published successfully!' : 'Draft saved successfully!');
  ...
  // Allow all authenticated users to edit their blog posts
  ...
  const isAuthor = post && user && (
    post.userId === user.id ||
    user.role === 'admin' ||
    (user.metadata?.globalRoles || []).some((role: string) =>
      ...
      <>
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          ...
          >
            ...
            <p className="text-text-secondary mb-6">You need to be logged in to edit a blog post.</p>
            <Button
      ...
      <>
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          ...
          >
            ...
            <p className="text-text-secondary mb-6">You need the blogger role to edit posts.</p>
            <Button
      ...
      <>
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          ...
          >
            <PenSquare size={48} className="text-gray-300 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-text-primary mb-4">Access Denied</h2>
            <p className="text-text-secondary mb-6">You can only edit your own posts.</p>
            <Button
              onClick={() => navigate('/blog/my-posts')}
              className="bg-primary-lime hover:bg-primary-lime/90 text-white"
            >
