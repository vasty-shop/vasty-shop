<p align="center">
  ...
  <p align="center">
    <a href="https://vasty.shop"><strong>🌐 Попробовать живое демо →</strong></a>
  </p>
</p>
...
<p align="center">
  <a href="https://vasty.shop">Живое демо</a> |
...
- [Что такое Vasty Shop?](#что-такое-vasty-shop)
- [Почему Vasty Shop?](#почему-vasty-shop-сравнение)
- [Панель продавца](#панель-продавца)
- [Быстрый старт](#быстрый-старт)
- [Возможности](#возможности)
- [Управление доставкой](#управление-доставкой)
- [Конструктор витрины](#конструктор-витрины)
...
Каждый продавец получает самообслуживаемую панель управления с KPI в реальном времени (выручка, заказы, товары, клиенты), разбивкой доходов — валовые продажи, стоимость доставки, чистая прибыль — и полным управлением заказами, товарами и одобрениями.

![Панель продавца](./docs/screenshots/VastyDash.png)
...
### Ядро E-Commerce
Всё необходимое для работы онлайн-магазина — каталог, корзина, оформление заказа и послепродажные процессы.
...
### Платежи и финансы
Мультипровайдерные платежи с автоматическими комиссиями платформы, выплатами продавцам и финансовым учётом.
...
### Маркетинг и рост
Встроенные инструменты генерации спроса — сторонние плагины не требуются.
...
### Операции
Процессы офлайн-торговли и фулфилмента для продавцов, работающих не только в вебе.
...
### Платформа
Базовые возможности, на которых работают все остальные модули.
...
Вся логистика платформы в одном месте: настройка способов и зон доставки, регистрация и управление партнёрами доставки, отслеживание активных отправлений и KPI — среднее время доставки, количество отправлений в пути — одним взглядом.

![Управление доставкой](./docs/screenshots/VastyDelivery.png)

## Конструктор витрины

Продавцы создают собственную витрину с помощью конструктора страниц drag-and-drop — hero-баннеры, рекомендуемые товары, категории, отзывы и кастомные страницы — с живым предпросмотром, который отражает каждое изменение мгновенно.

![Конструктор витрины](./docs/screenshots/VastyAppPreview.png)
...
| **Платежи** | Stripe, Stripe Connect, PayPal |
...
Vasty Shop спроектирован для самостоятельного хостинга. Запускайте его на любом хосте с Docker — VPS, Render/Railway/Fly или собственном кластере Kubernetes.

**Типичные сценарии развёртывания:**

- **Docker на одном хосте** — входящий в комплект `docker-compose.yml` готов к продакшену с небольшими правками env (смена build-таргетов, использование управляемых томов, установка reverse-прокси перед ним).
- **Managed Postgres + Redis** — направьте `DATABASE_URL` / `REDIS_HOST` на управляемый инстанс и запустите только контейнеры backend/frontend.
- **Настройка провайдеров** — хранилище, email, платежи, поиск, SMS и AI подключаются через переменные окружения. Полный каталог провайдеров и инструкции по настройке: [`backend/docs/providers/`](backend/docs/providers).

**Чек-лист перед запуском:**

- [ ] Смените пароль администратора по умолчанию (`admin@vasty.shop` / `admin123`)
- [ ] Установите `JWT_SECRET` в надёжное случайное значение
- [ ] Установите `NODE_ENV=production` в бекенде
- [ ] Настройте хотя бы одного email-провайдера для транзакционных писем
- [ ] Настройте провайдер хранилища (S3/R2/MinIO) для загружаемых медиафайлов
- [ ] Уберите секреты `backend/.env` из репозитория и системы контроля версий

## История изменений

См. [CHANGELOG.md](CHANGELOG.md) для истории версий и заметных изменений.
